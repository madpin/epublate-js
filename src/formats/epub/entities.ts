/**
 * Pre-parse XHTML entity normalization (browser-only concern).
 *
 * The Python tool uses `lxml` with `resolve_entities=False`, which
 * keeps named XHTML entities like `&nbsp;` / `&copy;` as
 * `etree.Entity` nodes that round-trip to the exact original bytes.
 *
 * `DOMParser`, our browser equivalent, has no DTD subsystem: it
 * rejects any entity reference outside the five XML built-ins
 * (`&amp;`, `&lt;`, `&gt;`, `&quot;`, `&apos;`). To stay compatible
 * with real-world XHTML 1.1 ePubs we **expand named entities** to
 * their literal Unicode characters before parsing.
 *
 * Two practical consequences:
 *
 *   1. Rendered output is unchanged: every reader (browser, Kindle,
 *      Calibre, Apple Books) treats `&copy;` and `©` identically. The
 *      visual round-trip property holds.
 *   2. Byte-identical round-trip of named entities does *not* hold
 *      anymore — we serialize as the literal Unicode character. The
 *      `EpubAdapter`-equivalent's contract is therefore "structural
 *      and visual identity"; "byte-identical XHTML" is downgraded.
 *      This matches the behavior the Python adapter already has for
 *      its TEXT_ENTITY_EXPANSIONS set, just over a wider catalogue.
 *
 * If we ever need byte-identical preservation of named entities, the
 * fix is a custom XML parser (or a sax-style `htmlparser2`) — out of
 * scope for v1.
 *
 * The map below is the union of the W3C XHTML 1.1 named entity sets
 * (Latin-1, Special, Symbol). It's static so V8 can fold the lookup
...
const ENTITY_REF_RE = /&([a-zA-Z][a-zA-Z0-9]*);/g;

/**
 * Map of named XHTML entity → literal Unicode character.
 * Derived from XHTML 1.1's HTMLlat1, HTMLsymbol, HTMLspecial DTDs.
 */
export const XHTML_NAMED_ENTITIES: Record<string, string> = {
  // Latin-1
  nbsp: "\u00a0",
  iexcl: "\u00a1",
  cent: "\u00a2",
  ...
  rsaquo: "\u203a",
  euro: "\u20ac",
  // Symbol (subset; the rest are Greek + math we rarely see in prose ePubs but harmless to add)
  fnof: "\u0192",
  Alpha: "\u0391",
  Beta: "\u0392",
  Gamma: "\u0393",
